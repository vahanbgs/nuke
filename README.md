...
what it cannot. `crates/nuke-transpile` writes JSON, YAML, TOML and XML:
...
shape refuses, and [`docs/xml.md`](docs/xml.md) records what a target with no data model at
all can still carry. The five other targets, the surface language and the tooling are still
ahead.
